package com.cynovo.sirius.util;

import java.text.NumberFormat;
import java.util.Locale;

public class NumberFormater {

	// 将数字以货币形式显示
	public static String currencyFormat(double d) {
		NumberFormat numberFormat = NumberFormat
				.getNumberInstance(Locale.CHINA);
		numberFormat.setMaximumFractionDigits(2);
		numberFormat.setMinimumFractionDigits(2);

		return numberFormat.format(d).replace(",", "");
	}

	// 将数字以货币形式显示
	public static String currencyFormat(String s) {
		if (s == null || s.isEmpty())
			s = "0.00";
		NumberFormat numberFormat = NumberFormat
				.getNumberInstance(Locale.CHINA);
		numberFormat.setMaximumFractionDigits(2);
		numberFormat.setMinimumFractionDigits(2);

		return numberFormat.format(Double.valueOf(s)).replace(",", "");
	}

	/**
	 * 将支付金额转化成12位的字符串
	 * 
	 * @param number
	 * @return 12位的字符串
	 */
	public static String twelveNumber(String number) {
		return String.format(Locale.getDefault(), "%012d", (long) Double
				.parseDouble((BasicArithmetic.multi(number, "100"))));
	}

	/**
	 * 将12位的字符串转化为支付金额
	 * 
	 * @param money
	 * @return 字符的金额
	 */
	public static String MoneyFromTwelveNumber(String money) {
		return NumberFormater
				.currencyFormat((BasicArithmetic.div(money, "100")));
	}

	/**
	 * 将支付流水号转为6位字符串
	 * 
	 * @param number
	 * @return
	 */
	public static String sixNumber(String number) {
		return String.format(Locale.getDefault(), "%06d",
				Integer.parseInt(number));
	}
}
